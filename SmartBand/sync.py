...
previous_hr_status = None
previous_spo2_status = None
...
def update_local_settings(local_cursor, settings):
    ...
    update_query = """UPDATE settings SET
                        ...
                        led_activation = %s, movement = %s, version = %s"""
...
def sync_status(local_cursor, rds_cursor, rds_db_connection, local_db_connection):
    ...
    global previous_hr_status
    global previous_spo2_status
    ...
    fall_status = local_status[0] 
    hr_unusual = local_status[1]
    spo2_unusual = local_status[2]
    if fall_status != previous_fall_status or hr_unusual != previous_hr_status or spo2_unusual != previous_spo2_status:
        current_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        if fall_status == 1 or hr_unusual == 1 or spo2_unusual == 1:
        ...
        previous_hr_status = hr_unusual
        previous_spo2_status = spo2_unusual
